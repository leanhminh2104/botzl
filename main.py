...
def initialize_group_info(bot, allowed_thread_ids):
    for thread_id in allowed_thread_ids:
        group_info = bot.fetchGroupInfo(thread_id).gridInfoMap.get(
            thread_id, None)
        if group_info:
...
def check_member_changes(bot, thread_id):
    current_group_info = bot.fetchGroupInfo(thread_id).gridInfoMap.get(
        thread_id, None)
    cached_group_info = bot.group_info_cache.get(thread_id, None)
    if not cached_group_info or not current_group_info:
        return [], []
    old_members = set(
        [member.split('_')[0] for member in cached_group_info["member_list"]])
    new_members = set(
        [member.split('_')[0] for member in current_group_info['memVerList']])
...
def create_welcome_image_with_avatar(member_name,
                                     avatar_url,
                                     font_folder,
                                     group_name,
                                     member_number,
    ...
    mask_draw.rounded_rectangle([(0, 0), (card_width, card_height)],
                                radius=20,
                                fill=255)
    ...
    text_area = Image.new("RGBA", (card_width - 160, card_height),
                          (0, 0, 0, 80))
    text_mask = Image.new("L", (card_width - 160, card_height), 0)
    text_mask_draw = ImageDraw.Draw(text_mask)
    text_mask_draw.rounded_rectangle([(0, 0), (card_width - 160, card_height)],
                                     radius=15,
                                     fill=255)
    ...
    # Load fonts
    try:
        title_font = ImageFont.truetype(os.path.join(font_folder, "arial.ttf"),
                                        35)
        regular_font = ImageFont.truetype(
            os.path.join(font_folder, "arial.ttf"), 30)
    except:
    ...
    try:
        ...
        frame_draw.ellipse([0, 0, frame_size, frame_size],
                           fill=(255, 165, 0, 255))
    ...
    draw.text((text_x, text_y),
              ...
              font=regular_font,
              fill=(255, 255, 255))
    draw.text((text_x, text_y + 30),
              f"Second Member: {member_number}",
              font=regular_font,
              fill=(255, 255, 255))

    draw.text((text_x, text_y + 70),
              "Hello And Hi:",
              font=title_font,
              fill=(0, 255, 255))
    draw.text((text_x, text_y + 110),
              f"•Welcome To: {member_name}",
              font=regular_font,
              fill=(255, 255, 255))
    draw.text((text_x, text_y + 150),
              f"Come With: {group_name}",
              font=regular_font,
              fill=(255, 255, 255))
    draw.text((text_x, text_y + 190),
              "A Place to Relax",
              font=regular_font,
              fill=(255, 255, 255))
    draw.text((text_x, text_y + 220),
...
def create_farewell_image_with_avatar(member_name,
                                      avatar_url,
                                      font_folder,
                                      group_name,
                                      background_path=None):
    ...
    draw.text((text_x, text_y + 80),
              "Farewell Message:",
              font=title_font,
              fill=(255, 69, 0))
    draw.text((text_x, text_y + 120),
              f"Good Bye: {member_name}",
              font=regular_font,
              fill=(255, 255, 255))
    draw.text((text_x, text_y + 160),
              f"Out The Group: {group_name}",
              font=regular_font,
              fill=(255, 255, 255))
    draw.text((text_x, text_y + 200),
              "See You Again 💔",
              font=regular_font,
              fill=(255, 255, 255))
...
def handle_group_member(bot,
    ...
    if joined_members:
        for member_id in joined_members:
            try:
                member_info = bot.fetchUserInfo(
                    member_id).changed_profiles[member_id]
                welcome_image_path = create_welcome_image_with_avatar(
                    member_info.displayName, member_info.avatar, "font",
                ...
                bot.sendLocalImage(welcome_image_path,
                                   thread_id,
                                   thread_type,
                                   message=messagesend,
                                   width=600,
        ...
        for member_id in left_members:
            try:
                ...
                farewell_image_path = create_farewell_image_with_avatar(
                    member_info.displayName, member_info.avatar, "font",
                    bot.group_info_cache[thread_id]['name'], background_path)
                messagesend = Message(
                ...
                bot.sendLocalImage(farewell_image_path,
                                   thread_id,
                                   thread_type,
                                   message=messagesend,
                                   width=600,
...
class Client(ZaloAPI):
    ...
    def start_member_check_thread(self, allowed_thread_ids):
        ...
        def check_members_loop():
            while True:
                for thread_id in allowed_thread_ids:
                    handle_group_member(self, None, None, thread_id,
                                        ThreadType.GROUP, self.background_path)
                time.sleep(1)
    ...
    def handle_message(self, mid, author_id, message, message_object,
                       thread_id, thread_type):
        try:
            message_text = message.text if isinstance(
                message, Message) else str(message)
            ...
            if isinstance(message, str):
                self.command_handler.handle_command(message, author_id,
                                                    message_object, thread_id,
                                                    thread_type)
            if self.del_enabled.get(thread_id, self.default_del_enabled):
                self.check_and_delete_links(author_id, message_text,
                                            message_object, thread_id)
...
## hàm xóa theo từ khóa
    ...
    def check_and_delete_links(self, author_id, message_text, message_object,
                               thread_id):
        # Tải bộ từ khóa
        ...
        if hit and author_id != self.bot_id:
            try:
                self.deleteGroupMsg(msgId=message_object.msgId,
                                    ownerId=author_id,
                                    clientMsgId=message_object.cliMsgId,
                                    groupId=thread_id)
                print(
                    f"Deleted message containing '{hit}' from {author_id} in group {thread_id}"
                )
            except Exception as e:
                print(f"Error while deleting message: {e}")
...
##################################
    ...
    def onMessage(self, mid, author_id, message, message_object, thread_id,
        ...
        threading.Thread(target=self.handle_message,
                         args=(mid, author_id, message, message_object,
                               thread_id, thread_type)).start()
